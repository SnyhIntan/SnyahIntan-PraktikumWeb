...
if (isset($_POST['update'])) {
    ...
    if ($stmt->execute()) {
        ...
        $rows = [];
        if (($handle = fopen('pembelian.csv', 'r')) !== FALSE) {
        ...
        // Cari data dengan ID yang sama dan update
        foreach ($rows as &$row) {
            if ($row[0] == $id) { // Asumsikan ID ada di kolom pertama
                $row[1] = $nama_produk;
                $row[2] = $harga;
                $row[3] = $quantity;
                $row[4] = $jumlah;
        ...
        // Tulis ulang CSV
        $csv_file = fopen('pembelian.csv', 'w');
        foreach ($rows as $row) {
