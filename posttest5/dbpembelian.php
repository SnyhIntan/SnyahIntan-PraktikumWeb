...
//     if ($stmt->execute()) {
//         echo "Data pembelian telah dihapus.";
//     } else {
...
if (isset($_POST['tambah'])) {
    ...
    $insert_sql = "INSERT INTO pembelian (nama, harga, quantity, jumlah, tanggal) VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($insert_sql);
    $stmt->bind_param('sdiis', $nama, $harga, $quantity, $jumlah, $tanggal);

    if ($stmt->execute()) {
        echo "Data berhasil ditambahkan ke database.";

        // Menyimpan data ke CSV
        $csv_file = fopen('pembelian.csv', 'a');
        $csv_data = ['sdiis', $nama, $harga, $quantity, $jumlah, $tanggal];
        fputcsv($csv_file, $csv_data);
        fclose($csv_file);

        echo "Data juga telah ditambahkan ke pembelian.csv.";
    } else {
